            ...
            with (open(splitext(self.header['file'])[0] + '.csv','w',newline='') if version_info[0] > 2 else open(splitext(self.header['file'])[0] + '.csv','wb')) as output_file:
                ...
                for channel in self.channels:
                    for trial in range(self.header['n_trials']):
                        ...
                        if mepconfig.remove_rejected_mep and (any(channel['header']['rejected'].values()) and any([channel['rejected'][rejection_criterion][trial] if channel['rejected'][rejection_criterion] is not None else False for rejection_criterion in channel['rejected']])):
