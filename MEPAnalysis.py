...
try:
    ...
    if fileType == '.adibin':
        ...
        for i in range(fileHeader['nChannels']):
            ...
            for j in range(processing['skipNTrials'],processing['totalNTrials']):
                ...
                if processing['detrend']:
                    channels[i]['trialData'][j]['data'] = list(detrend([channelData.popleft() for _ in range(samplesPerTrial)],type=processing['detrend']))
                else:
                    channels[i]['trialData'][j]['data'] = [channelData.popleft() for _ in range(samplesPerTrial)]
                channels[i]['trialData'][j]['ptp'] = [None,None,None]
                channels[i]['trialData'][j]['window'] = [None,None,None]
        ...
        # Assume mep file is being loaded to query analysis
        temp['peakAnalysis']['query'] = temp['peakAnalysis']['query'] = True
        # Ask whether to re-analyse the data
        ...
        for i in range(fileHeader['nChannels']):
            ...
            for j in range(len(channels[i]['trialData'])):
                ...
                if processing['peakAnalysis']['auto'] and processing['peakAnalysis']['analyse']:
                    ...
                    PEAKS, VALLEYS = extractPeaksValleys(channels[i]['trialData'][j]['data'],peakTimeIndices,prominenceCriterion)
    ...
    if processing['peakAnalysis']['query'] and processing['peakAnalysis']['analyse']:
        ...
        for i in range(fileHeader['nChannels']):        
            queryData(channels[i]['header']['title'],channels[i]['trialData'],fileHeader['samplingTickrate'],fileHeader['pretriggerTime'])
                ...
                for i in range(fileHeader['nChannels']):
                    for j in range(len(channels[i]['trialData'])):
                        ...
                        if processing['peakAnalysis']['analyse'] and (processing['peakAnalysis']['query'] or processing['peakAnalysis']['auto']):
                            if channels[i]['trialData'][j]['ptp'][0]:
                                trialString += [channels[i]['trialData'][j]['ptp'][0],] + [convertSampleToTime(x) for x in channels[i]['trialData'][j]['ptp'][1:]]
                            else:
                                trialString += ['','','']
                        if processing['windowAnalysis']['analyse'] and (processing['windowAnalysis']['query'] or processing['windowAnalysis']['auto']):
                            if channels[i]['trialData'][j]['window'][0]:
                                trialString += [channels[i]['trialData'][j]['window'][0],] + [convertSampleToTime(x) for x in channels[i]['trialData'][j]['window'][1:]]
                            else:
                                trialString += ['','','']
